...
Retryable errors carry a `Retry-After` header **and** `error.retryAfterSeconds` (same value, in seconds): `RATE_LIMITED` (429), `TOO_MANY_FAILED_ATTEMPTS` / `MFA_TOO_MANY_ATTEMPTS` (429), `IDEMPOTENCY_KEY_IN_FLIGHT` (409), `DEPENDENCY_UNAVAILABLE` (503), and `PASSWORD_VERIFY_BUSY` (503). Honour it before retrying; nothing else in this document is worth retrying without a code change.
