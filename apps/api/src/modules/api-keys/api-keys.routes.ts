...
const MintedApiKey: JsonSchema = {
  ...
  properties: {
    ...
    warning: {
      type: "string",
      description:
        "Human-readable restatement of the one-time-only warning above.",
    },
  },
  ...
  app.get(
    "/:id/api-keys",
    {
      schema: {
        ...
        description:
          "Returns key metadata only. The raw key value is unrecoverable after creation.",
        params: {
          type: "object",
    ...
    "/:id/api-keys",
    {
      schema: {
        ...
        body: {
          ...
          properties: {
            name: {
              ...
              maxLength: 120,
              description:
                'Human-readable label, e.g. "CI server", "Staging worker".',
            },
            scopes: {
              type: "array",
              items: { type: "string" },
              description:
                'Defaults to `["*"]` (full access). Use `auth:read`, `billing:write`, etc., to scope.',
            },
            expiresAt: {
              type: "string",
              format: "date-time",
        ...
        response: {
          201: ok(
            MintedApiKey,
            "The newly minted key. Store `rawKey` now, it is shown exactly once.",
          ),
          ...errs({
            400:
