...
const HOSTED_PROVIDERS = registryNames.filter(
  (n) => getModule(n)!.capabilities.checkout,
);
...
const CreateApplicationBody = z.object({
  ...
  authConfig: z
    .object({
      methods: z
        .array(z.enum(["password", "google", "github", "magic_link"]))
        .optional(),
...
const ListQuery = z
  .object({ tenantId: z.string().min(1).optional() })
...
export async function applicationsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: stripApplicationSecrets(await applicationsService.get(id)),
      };
    },
  );
  ...
  app.post(
        ...
        body: {
          ...
          properties: {
            ...
            slug: {
              ...
              maxLength: 40,
              description:
                "URL-safe lowercase identifier. Letters, digits, hyphens only.",
            },
            environment: {
              type: "string",
            ...
            enableBilling: {
              type: "boolean",
              description:
                "Provision with the billing surface enabled. Defaults false.",
            },
            authConfig: {
              type: "object",
              properties: {
        ...
        response: {
          ...
          ...errs({
            ...
            403:
              ...
              "ADMIN_IP_UNVERIFIABLE — an allowlist is set but the caller arrived through a proxy " +
              "the API cannot identify; or " +
    ...
    async (req, reply) => {
      ...
      const application = await applicationsService.create({
        ...
        ...(input.billingProvider !== undefined && {
          billingProvider: input.billingProvider,
        }),
        ...(input.enableBilling !== undefined && {
