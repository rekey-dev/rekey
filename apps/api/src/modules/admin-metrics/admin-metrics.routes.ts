...
// TS return types in admin-metrics.service.ts (see the JSDoc there for the
...
const OverviewSchema: JsonSchema = {
  ...
  properties: {
    ...
    mrrCents: {
      type: "integer",
      description:
        "SUM of ACTIVE subscriptions, YEAR-interval plans converted to a monthly figure.",
    },
    mrrCapped: {
      type: "boolean",
      description:
...
const ServicesSchema: JsonSchema = {
  type: "object",
  description:
    "Live DB + Redis ping, outbound webhook 24h success rate, oldest unprocessed inbound webhook.",
  properties: {
    api: {
      type: "object",
      properties: {
    ...
    database: {
      ...
      properties: {
        ...
        latencyMs: {
          type: "integer",
          nullable: true,
          description: "Null when the ping itself failed.",
        },
      },
    ...
    redis: {
      ...
      properties: {
        status: {
          ...
          description:
            "`not_configured` when this deployment has no Redis (e.g. NODE_ENV=test).",
        },
    ...
    webhookDeliverySuccessRate24h: {
      ...
      nullable: true,
      description:
        "succeeded / total over the last 24h. Null when there were zero deliveries.",
    },
    oldestUnprocessedWebhookAgeSeconds: {
      type: "integer",
      nullable: true,
      description:
        "Age of the oldest inbound WebhookEvent with `processedAt: null`. Null when none are pending.",
    },
  },
  required: [
    "api",
...
const RetentionSchema: JsonSchema = {
  ...
  properties: {
    ...
    signupTrend14d: {
      type: "array",
      description:
        "One entry per UTC day that had at least one signup, missing days are not zero-filled.",
      items: {
        type: "object",
        properties: {
...
const TenantSummaryItem: JsonSchema = {
  ...
  properties: {
    ...
    mrrCapped: {
      type: "boolean",
      description:
        "True when this tenant's MRR sum saturated the 10,000-row read cap (lower bound).",
    },
    ...
    lastActivityAt: {
      ...
      nullable: true,
      description:
        "Most recent ApiRequestLog row across the tenant's applications.",
    },
  },
  required: [
    "id",
...
const EndUserSummaryItem: JsonSchema = {
  ...
  properties: {
    ...
    lastSeenAt: {
      ...
      nullable: true,
      description:
        "Most recent RefreshToken.createdAt, a sign-in/refresh proxy.",
    },
  },
  required: [
    "id",
...
const TenantUserSummaryItem: JsonSchema = {
  ...
  properties: {
    ...
    membershipCount: {
      type: "integer",
      description: "Number of workspaces this operator belongs to.",
    },
  },
  required: [
    "id",
...
const SecurityEventSummaryItem: JsonSchema = {
  ...
  properties: {
    ...
    tenantName: {
      ...
      nullable: true,
      description:
        "Resolved from `tenantId`; null if unset or the tenant is gone.",
    },
    ...
    metadata: {
      type: "object",
      description: "Event-specific detail; shape varies by `type`.",
    },
    createdAt: { type: "string", format: "date-time" },
  },
  required: [
...
const WebhookDeliverySummaryItem: JsonSchema = {
  type: "object",
  description:
    "An outbound delivery attempt to a tenant's registered WebhookEndpoint.",
  properties: {
    id: { type: "string" },
...
const CreditLiabilitySchema: JsonSchema = {
  type: "object",
  description:
    "SUM(CreditBalance.balance) deployment-wide + top-20 applications by outstanding balance.",
  properties: {
    totalOutstanding: {
      type: "integer",
      description: "Unit-less, each application defines what a credit means.",
    },
    perApp: {
      type: "array",
      items: {
...
const LockedAccountsSchema: JsonSchema = {
  ...
  properties: {
    total: {
      type: "integer",
      description: "Count of locked end-user accounts.",
    },
    accounts: {
      type: "array",
      items: {
        ...
        properties: {
          id: {
            type: "string",
            description:
              "EndUser id, or a synthetic `<applicationId>:<email>` when the row is gone.",
          },
          applicationId: { type: "string" },
          applicationSlug: { type: "string" },
          email: { type: "string", format: "email" },
          failedAttempts: {
            type: "integer",
            description:
              "Reports the policy threshold, not a live counter (the counter is cleared at lock time).",
          },
    ...
    operatorsTotal: {
      type: "integer",
      description: "Count of locked OPERATOR accounts.",
    },
    operators: {
      type: "array",
      items: {
        ...
        properties: {
          ...
          workspaces: {
            type: "array",
            description:
              "Every workspace this operator belongs to, who else can still get in.",
            items: {
              type: "object",
              properties: {
                tenantId: { type: "string" },
...
const EmailDeliverabilitySchema: JsonSchema = {
  type: "object",
  description:
    "EmailLog status counts over the last 24h + 7d, plus top-5 applications by error count (7d).",
  properties: {
    last24h: {
      type: "object",
      properties: {
...
const WebhookEndpointHealthItem: JsonSchema = {
  ...
  properties: {
    ...
    successRate: {
      ...
      nullable: true,
      description:
        "succeeded / (succeeded+failed+pending) over the last 24h. Null when there were zero deliveries.",
    },
  },
  required: [
    "endpointId",
...
const PaymentsByAppItem: JsonSchema = {
  ...
  properties: {
    ...
    successRate: {
      ...
      nullable: true,
      description:
        "succeeded / (succeeded+failed) over the last 30 days. Null when both are zero.",
    },
    volumeCents: {
      type: "integer",
      description:
        "SUCCEEDED payment volume in minor currency units over the last 30 days.",
    },
  },
  required: [
    "applicationId",
...
export async function adminMetricsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    "/overview",
    {
      schema: {
        ...
        description:
          "Tenants, applications, end-users, orgs, subscriptions, payments, MRR, webhook + API-request health over the last 24h/30d.",
        response: {
  ...
  app.get(
    "/retention",
    {
      schema: {
        ...
        description:
          "End-user & operator active counts over 24h/7d/30d (refresh-token proxy) plus a 14-day end-user signup histogram.",
        response: {
          200: ok(
            RetentionSchema,
            "Active-user counts + the 14-day signup trend.",
          ),
          ...errs(SUPER_ADMIN_ERRORS),
        },
      },
    ...
    async () => ({
      success: true,
      data: await adminMetricsService.retention(),
    }),
  );
  ...
  app.get(
      ...
      schema: {
        ...
        response: {
          200: okPage(
            ...
            "A page of per-application summaries.",
          ),
          ...errs(LIST_QUERY_ERRORS),
        },
      },
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.applications(query),
      };
    },
  );
  ...
  app.get(
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.tenantUsers(query),
      };
    },
  );
  ...
  app.get(
    "/security-events",
    {
      schema: {
        ...
        response: {
          200: okPage(
            SecurityEventSummaryItem,
            "A page of security events, newest first by default.",
          ),
          ...errs(LIST_QUERY_ERRORS),
        },
      },
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.securityEvents(query),
      };
    },
  );
  ...
  app.get(
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.apiRequests(query),
      };
    },
  );
  ...
  app.get(
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.subscriptions(query),
      };
    },
  );
  ...
  app.get(
    "/webhook-events",
    {
      schema: {
        ...
        response: {
          200: okPage(
            WebhookEventSummaryItem,
            "A page of inbound webhook events.",
          ),
          ...errs(LIST_QUERY_ERRORS),
        },
      },
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.webhookEvents(query),
      };
    },
  );
  ...
  app.get(
    "/webhook-deliveries",
    {
      schema: {
        ...
        response: {
          200: okPage(
            WebhookDeliverySummaryItem,
            "A page of outbound webhook deliveries.",
          ),
          ...errs(LIST_QUERY_ERRORS),
        },
      },
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.webhookDeliveries(query),
      };
    },
  );
  ...
  app.get(
      ...
      schema: {
        ...
        response: {
          200: ok(
            ...
            "Outstanding prepaid credit liability.",
          ),
          ...errs(SUPER_ADMIN_ERRORS),
        },
      },
    ...
    async () => ({
      success: true,
      data: await adminMetricsService.creditLiability(),
    }),
  );
  ...
  app.get(
    "/locked-accounts",
    {
      schema: {
        ...
        response: {
          200: ok(
            LockedAccountsSchema,
            "End-users and operators currently locked out.",
          ),
          ...errs(LIST_QUERY_ERRORS),
        },
      },
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await adminMetricsService.lockedAccounts(query),
      };
    },
  );
  ...
  app.get(
    ...
    async () => ({
      success: true,
      data: await adminMetricsService.emailDeliverability(),
    }),
  );
  ...
  app.get(
    "/webhook-endpoint-health",
    {
      schema: {
        ...
        description:
          "Outbound deliveries aggregated by endpoint, success rate + retry-storm flag. Sorted by failure count.",
        response: {
          200: okArray(
    ...
    async () => ({
      success: true,
      data: await adminMetricsService.webhookEndpointHealth(),
    }),
  );
  ...
  app.get(
    "/payments-by-app",
    {
      schema: {
        ...
        description:
          "Payments aggregated by application + status over the last 30 days. Surfaces apps whose provider integration is failing.",
        response: {
          200: okArray(
            PaymentsByAppItem,
    ...
    async () => ({
      success: true,
      data: await adminMetricsService.paymentsByApp(),
    }),
  );
}
