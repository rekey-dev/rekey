...
/**
 * Plan create, a **closed** field list.
 *
 * `.strict()`, and it is the whole point of this schema. `plansService.create`
 * implements LICENSE / USAGE / CREDIT config and `trialDays`; this route does
 * not expose any of it, and zod's default is to STRIP what it does not know.
 * So a caller sending `kind: 'LICENSE'` or `creditsAmount: 500` got 201 back
 * describing a SUBSCRIPTION plan it never asked for. `rekey plans create
 * --kind LICENSE --credits-amount 500` printed a tick and made the wrong plan;
 * nothing anywhere said otherwise until someone read the pricing page.
 *
 * Checked HERE rather than with `additionalProperties: false` on the route's
 * JSON schema, for the same reason as `SetLimitsBody` in tenants.routes.ts:
 * Fastify's AJV runs with `removeAdditional: true`, so declaring it there
 * would delete the unknown key before this parse could refuse it, which is the
 * silent 201 again by another path.
 *
 * The tenant route implements the other kinds: see `CreatePlanBody` in
 * tenant-applications.routes.ts. Widen this schema only by implementing the
 * field, never to make a caller's request go quiet.
 */
const CreateBody = z
  .object({
...
export async function plansRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (req) => {
      ...
      const [items, total] = await Promise.all([
        plansService.listForApplication(id, includeInactive ?? false, {
          take,
          skip,
        }),
    ...
    "/:id/plans",
    {
      schema: {
        ...
        description:
          ...
          "Creates SUBSCRIPTION plans only, and the body is closed: a key this route does not " +
          "implement (`kind`, `licenseKind`, `creditsAmount`, `trialDays`, or a typo) is " +
          "refused, not ignored. LICENSE, USAGE and CREDIT plans are created on the tenant " +
          "route, POST /api/v1/tenant/applications/:id/plans.",
        params: {
          type: "object",
          properties: { id: { type: "string" } },
          required: ["id"],
        ...
        response: {
          ...
          ...errs({
            ...
            // Asking for one of those kinds is now refused by the body schema
            // rather than dropped, so the caller learns that here instead of
            // from the plan they did not want. See `CreateBody`.
            400:
              "VALIDATION_ERROR — a field failed schema validation, or the body carries a key " +
              "this route does not implement (`kind`, `licenseKind`, `creditsAmount`, " +
              "`trialDays`, ...), which is refused rather than ignored; or PLAN_SLUG_INVALID — " +
              "the slug is not URL-safe; or PLAN_AMOUNT_INVALID — `amount` is negative.",
            ...SUPER_ADMIN_ERRORS,
            404: "APPLICATION_NOT_FOUND — no application with that id.",
  ...
  app.post(
    ...
    async (req) => {
      ...
      return {
        success: true,
        data: await plansService.registerWithProvider(id, slug),
      };
    },
  );
}
