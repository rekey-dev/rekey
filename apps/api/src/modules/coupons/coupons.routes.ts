 ...
 *
 * The user JWT is the authorizer: per-user redemption limits are evaluated
 * against `req.endUser`.
 *
 * `billing:read` binds SECRET-key callers only. `requireScope` no-ops for a
 * publishable request since a publishable key carries no scopes to check; the
 * origin allowlist and the user session constrain it instead.
 */
  ...
  app.post(
    "/validate",
    {
      schema: {
        ...
        response: {
          200: ok(
            ref("ValidateCouponResult"),
            "The discount this coupon resolves to for this plan.",
          ),
          ...errs({
            400:
    ...
    async (req) => {
      ...
      const plan = await plansService.getBySlug(
        req.application!.id,
        body.planSlug,
      );
      ...
      return {
        ...
        data: {
          ...
          // the publishable key and a user token (a pricing page), so it must
          // not leak `maxRedemptions`, `maxRedemptionsPerUser`, the operator's
          // private `metadata`, the internal id, or the full plan allow-list.
          // The limits in particular would tell a scraper exactly how much of
          // a campaign is left to burn. A buyer only needs the code, the kind
          // of discount, and its size to render "15% off"; the money they care
          // about is already `discountAmount` / `amountAfterDiscount` below.
          coupon: {
