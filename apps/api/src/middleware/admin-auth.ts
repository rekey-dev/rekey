 ...
 *
 * That gate needs an address that is the caller's own. Behind a proxy the API
 * cannot identify, `request.ip` is the proxy's, shared by everyone behind it,
 * so an allowlist naming it (or the private range it sits in) would admit them
 * all. A configured allowlist therefore refuses an unvouched address outright.
 */
...
import type { ClientIpPolicy } from "../lib/client-ip.js";
...
/**
 * The one startup warning for an allowlist the deployment cannot enforce:
 * configured, but with no way to identify a proxy, so every admin call from
 * behind one is refused. Null when there is no allowlist or the deployment has
 * said how its proxy identifies itself.
 */
export function adminIpAllowlistWarning(policy: ClientIpPolicy): string | null {
  if (adminIpAllowlist().length === 0) return null;
  if (policy.proxySecret) return null;
  if (typeof policy.internalCallers === "number") return null;
  return (
    "[admin] ADMIN_IP_ALLOWLIST is set, but API_PROXY_SECRET is not, so a request arriving " +
    "through a proxy on a private network cannot be traced to its client. The allowlist cannot " +
    "check an address it does not have, so /api/v1/admin/* answers 403 ADMIN_IP_UNVERIFIABLE for " +
    "that traffic. Set API_PROXY_SECRET and have the proxy send it as X-Rekey-Proxy-Secret, or " +
    "clear ADMIN_IP_ALLOWLIST; see DEPLOY.md."
  );
}
...
export async function requireSuperAdmin(
  ...
  // allowlist learns nothing about whether their key was right.
  const allowlist = adminIpAllowlist();
  if (allowlist.length > 0) {
    // `request.ip` is only the caller's own address when lib/client-ip.ts
    // vouched for it. Unvouched it is a proxy we cannot identify, shared by
    // everyone behind it, so matching it would admit them all, which is the
    // thing the allowlist was set up to prevent.
    if (!request.clientIpVouched) {
      throw new RekeyError({
        statusCode: 403,
        code: "ADMIN_IP_UNVERIFIABLE",
        message:
          "Admin endpoints are not available from an address this API cannot verify.",
        fix:
          "This deployment restricts /api/v1/admin/* to ADMIN_IP_ALLOWLIST, but the request " +
          "arrived through a proxy the API cannot identify, so the address it sees is the " +
          "proxy's and is shared by everyone behind it. Set API_PROXY_SECRET and have the " +
          "proxy send it as X-Rekey-Proxy-Secret (see DEPLOY.md), or clear ADMIN_IP_ALLOWLIST " +
          "and restart the API.",
      });
    }
    if (!ipMatchesAllowlist(request.ip, allowlist)) {
      throw new RekeyError({
        statusCode: 403,
  ...
  if (!presented) {
    throw new RekeyError({
      ...
      message:
        "Admin endpoints require an Authorization: Bearer <SUPER_ADMIN_KEY> header.",
