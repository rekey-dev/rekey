 ...
 * revalidation in actions, plus a client refresh after an action redirect lands.
 * These tests pin both halves, and read the installed Next so that an upgrade
 * which moves the ground under either one fails here.
 */
...
describe('panel source', () => {
  ...
  it('no file both revalidates and redirects', () => {
    const offenders = sourceFiles(srcDir).filter((f) => {
      const c = code(f);
      return /revalidate(Path|Tag)\s*\(/.test(c) && /\bredirect\s*\(/.test(c);
    });
