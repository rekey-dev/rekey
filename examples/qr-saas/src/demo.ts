/**
 * End-to-end driver for the QR SaaS — exercises Rekey as a real integrator:
 *
...
/**
 * Rekey enforces a global 100 req/min rate limit with no env override and no
...
/** Capture a RekeyError (or QrError) thrown by an async fn. */
async function expectError(fn: () => Promise<unknown>): Promise<{ code?: string; status?: number } | null> {
  try {
    await fn();
    return null;
  } catch (e) {
    if (e instanceof RekeyError) return { code: e.code, status: e.statusCode };
...
async function main(): Promise<void> {
  ...
  // SDK smoke test.
  await section('SDK connectivity (rekey.applications.me)');
  ...
  const signUp = await rekey.auth.signUp({ email, password: 'correct-horse-battery-staple' });
  ...
  const signInOutcome = await rekey.auth.signIn({ email, password: 'correct-horse-battery-staple' });
  ...
  const current = await rekey.auth.getCurrentUser(accessToken);
  ...
  const refreshed = await rekey.auth.refresh(refreshToken);
  ...
  // (Rekey has no auto-assigned default plan — a freemium tier is a real
  // ACTIVE subscription, even at amount 0.)
  await activatePlan(rekey, config, accessToken, 'free');
  const ent0 = await resolveEntitlements(rekey, subject);
  ...
  for (let i = 0; i < FREE_MAX_QRS; i++) {
    created.push(await qrService.create(rekey, subject, { destination: `https://example.com/${i}`, title: `QR ${i}` }));
  }
  ok(`created ${FREE_MAX_QRS} QRs (the Free cap)`, created.length === FREE_MAX_QRS);

  const overCap = await expectError(() =>
    qrService.create(rekey, subject, { destination: 'https://example.com/over' }),
  );
  ok('creating past the cap is rejected (402)', overCap?.status === 402 && overCap.code === 'QR_LIMIT_REACHED', overCap?.code);

  const listed = await qrService.list(rekey, subject);
  ok('qrService.list returns owned QRs', listed.length === FREE_MAX_QRS);

  const target = created[0]!;
  const updated = await qrService.updateDestination(rekey, subject, target.id, 'https://example.com/edited');
  ...
  for (let i = 0; i < recordTo; i++) {
    await qrService.recordScan(rekey, target);
    await sleep(120);
  }
  const aggUser = await rekey.usage.aggregate({ meterSlug: METER_QR_SCANS, endUserId });
  ...
  const capHit = await expectError(() => qrService.recordScan(rekey, target));
  ...
  const entPro = await resolveEntitlements(rekey, subject);
  ...
  const proQr = await qrService.create(rekey, subject, { destination: 'https://example.com/pro' });
  ok('can create beyond the Free cap on Pro', !!proQr.id);

  // Analytics is gated; should work now.
  const analytics = await qrService.analytics(rekey, subject, target.id);
  ...
  const postUpgradeScan = await expectError(() => qrService.recordScan(rekey, proQr));
  ...
  const packCheckout = await rekey.billing.createCheckout(accessToken, {
  ...
  const balance = await rekey.credits.getBalance({ endUserId });
  ok('credit balance granted after purchase', balance.balance >= 500, `balance=${balance.balance}`);

  const consume = await rekey.credits.consume({ endUserId, amount: 10, description: 'bulk_generate', idempotencyKey: `demo-${Date.now()}` });
  ...
  const orgCreate = await rekey.organizations.create(accessToken, { name: 'Acme Team', slug: orgSlug });
  ...
  const switched = await rekey.organizations.switch(accessToken, orgId);
  ...
  const orgWhoami = await rekey.auth.getCurrentUser(orgAccessToken);
  ...
  const orgCheckout = await rekey.billing.createCheckout(orgAccessToken, {
    planSlug: PLAN_PRO,
  ...
  const orgEnt = await rekey.billing.getEntitlements(orgAccessToken, { organizationId: orgId });
  ...
  const teamQr = await qrService.create(rekey, orgSubject, { destination: 'https://example.com/team', title: 'Team QR' });
  ok('team QR created in the org workspace', teamQr.organizationId === orgId);
  await qrService.recordScan(rekey, teamQr);
  const orgAgg = await rekey.usage.aggregate({ meterSlug: METER_QR_SCANS, organizationId: orgId });
  ...
  const invite = await rekey.organizations.invite(orgAccessToken, orgId, { email: `bob+${Date.now()}@example.com`, role: 'MEMBER' });
  ok('organizations.invite returns a raw token to forward', !!invite.token, invite.invitation.id);

  // ---------- H. Sign-out ----------
  await section('H. Sign-out');
  const signedOut = await rekey.auth.signOut(refreshToken);
  ok('auth.signOut revokes the refresh token', signedOut.signedOut === true);
  const reuse = await expectError(() => rekey.auth.refresh(refreshToken));
