 ...
 *
 * The portal is operated by the ReliPay deployment's operator and serves the
 ...
 *
 *   RELIPAY_URL         — base URL of the ReliPay API (e.g. https://api.example.com)
 ...
 *
 * `@relipay/nextjs/server` reads `RELIPAY_SECRET` — this module bridges the
 ...
 * before any `@relipay/nextjs/server` helper runs; lib/session and
 * lib/actions both do.
 */
...
export function requireEnv(name: 'RELIPAY_URL' | 'RELIPAY_SECRET_KEY'): string {
  ...
  // examples/nextjs-saas and @relipay/nextjs.
  const value =
